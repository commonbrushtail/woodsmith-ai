...
/**
 * Presentational footer, driven entirely by `settings`. Shared by the public
 * Footer (server wrapper that fetches site settings) and the site-settings
 * preview adapter, so preview === production.
 *
 * @param {{ settings?: object }} props
 */
export default function FooterView({ settings = defaultFooterSettings }) {
  const siteSettings = settings || defaultFooterSettings
