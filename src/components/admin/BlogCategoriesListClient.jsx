...
export default function BlogCategoriesListClient({ categories }) {
  ...
  const handleDelete = async (id) => {
    startTransition(async () => {
      ...
      if (result.warning) {
        const confirmDelete = confirm(`${result.message} ต้องการลบหรือไม่? (บทความที่ใช้หมวดหมู่นี้จะถูกตั้งเป็นไม่มีหมวดหมู่)`)
        if (confirmDelete) {
          const forceResult = await deleteBlogCategory(id, { force: true })
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-[#e8eaef]">
              ...
              <th className="text-left px-[16px] py-[12px] font-['IBM_Plex_Sans_Thai'] text-[12px] font-semibold text-[#9ca3af] uppercase tracking-wider">ชื่อหมวดหมู่</th>
              <th className="text-left px-[16px] py-[12px] font-['IBM_Plex_Sans_Thai'] text-[12px] font-semibold text-[#9ca3af] uppercase tracking-wider">Slug</th>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={4} className="text-center py-[40px] font-['IBM_Plex_Sans_Thai'] text-[14px] text-[#9ca3af]">
                  {search ? 'ไม่พบหมวดหมู่ที่ค้นหา' : 'ยังไม่มีหมวดหมู่บทความ'}
                </td>
              </tr>
            ) : (
              filtered.map((cat, idx) => (
                <tr
                  ...
                  onClick={(e) => {
                    ...
                    router.push('/admin/blog-categories/edit/' + cat.id)
                  }}
                ...
                >
                  ...
                  <td className="px-[16px] py-[14px]">
                    <Link
                      href={`/admin/blog-categories/edit/${cat.id}`}
