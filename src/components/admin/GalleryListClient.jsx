...
export default function GalleryListClient({ galleries, totalCount }) {
  ...
  const allSelected = selectedRows.length === galleries.length && galleries.length > 0
  const someSelected = selectedRows.length > 0 && !allSelected

  function toggleSelectAll() {
    setSelectedRows(allSelected ? [] : galleries.map((g) => g.id))
  }
  ...
  const handleTogglePublished = (id, current) => {
    startTransition(async () => {
      await toggleGalleryPublished(id, !current)
      router.refresh()
    })
  }

  function renderPublishBadge(item) {
    return (
      <button onClick={() => handleTogglePublished(item.id, item.published)} className="cursor-pointer bg-transparent border-none p-0">
        {item.published ? (
