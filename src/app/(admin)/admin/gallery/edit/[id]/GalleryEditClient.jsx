...
export default function GalleryEditClient({ item }) {
  ...
  const [activeTab, setActiveTab] = useState(item.published ? 'published' : 'draft')
      ...
      <div className="flex gap-[24px] mt-[20px] flex-1 min-h-0 overflow-y-auto pb-[32px]">
        <div className="flex-1 flex flex-col gap-[24px] min-w-0">
            ...
            {imagePreview ? (
              <div className="flex flex-col gap-[8px]">
                <div className="relative w-full h-[200px] rounded-[8px] overflow-hidden bg-[#f3f4f6]">
                  <img src={imagePreview} alt="Gallery" className="w-full h-full object-cover" />
                </div>
                <button
                  type="button"
          ...
          <section className="bg-white rounded-[12px] border border-[#e8eaef] p-[24px] flex flex-col gap-[8px]">
            <label htmlFor="galleryCaption" className="font-['IBM_Plex_Sans_Thai'] text-[14px] font-medium text-[#1f2937]">
              Title / ชื่อ
            </label>
            <input
              id="galleryCaption"
