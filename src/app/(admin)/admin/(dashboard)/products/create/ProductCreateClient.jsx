...
export default function ProductCreateClient({ categories = [], variationGroups = [] }) {
  ...
  const [calculatorSizes, setCalculatorSizes] = useState([
    { label: '',  piecesPerBox: '', plankWidth: '', plankLength: '', installationPatterns: defaultPatterns }
  ])
    ...
    startTransition(async () => {
      ...
      if (pendingFiles.length > 0 && newId) {
        ...
        for (const pf of pendingFiles) {
          ...
          fileFormData.set('file', pf.file)
          const uploadResult = await uploadProductImage(newId, fileFormData)
          if (uploadResult.error) uploadErrors++
        }
        if (uploadErrors > 0) {
