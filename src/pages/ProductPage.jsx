...
const activeFilters = ['ไม้พื้นลามิเนต', 'ไม้ฝา', 'บานประตู', 'สำเร็จรูป']
      ...
      <div className="w-full bg-white py-[32px] lg:py-[48px]">
        <div className="max-w-[1212px] mx-auto w-full flex flex-col gap-[24px] items-center px-[16px]">
          ...
          <div className="flex flex-col sm:flex-row gap-[16px] lg:gap-[24px] w-full">
            {[
              { label: 'วัสดุก่อสร้าง', count: '250 รายการ', path: '/products/construction' },
              { label: 'ผลิตภัณฑ์สำเร็จ', count: '250 รายการ', path: '/products/finished' },
            ].map((cat) => (
              <Link
                key={cat.label}
                to={cat.path}
        ...
        <div className="flex-1 flex flex-col gap-[16px] lg:gap-[24px] w-full min-w-0">
          ...
          <div className="flex flex-wrap gap-[8px] items-center">
            ...
            {activeFilters.map((filter) => (
              <span
                key={filter}
                className="border border-[#e5e7eb] rounded-[6px] px-[10px] py-[6px] font-['IBM_Plex_Sans_Thai'] font-medium text-[12px] text-[#6b7280]"
              >
                {filter}
              </span>
            ))}
            <button className="bg-transparent border-none cursor-pointer p-0">
              <span className="font-['IBM_Plex_Sans_Thai'] font-medium text-[15px] text-black underline leading-[1.2]">
                ล้างทั้งหมด
              </span>
            </button>
          </div>
